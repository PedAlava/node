...
$(document).on('click', '#main-chat #paper-btn', function (e) {
        ...
        var dataIdss= $($(e.currentTarget).parent().parent().parent()).find("#nombre").val();
        var dataIds = $(this).parents('.chat-single-box').find('#nombre').val();
        var padre_tr = $(this).parents('.user-info');
        var dataUserName = padre_tr.find('#nombre').val();
        
        var dataUserName = document.querySelector('#nombre').getAttribute('value');
        alert(dataUserName)
        console.log(dataIds)
        console.log(dataUserName)
        fetch("/message", {
            method: 'POST', // or 'PUT'
            body: JSON.stringify({username: text,dataUserName:dataUserName}), // data can be `string` or {object}!
            headers:{
        ...
        alert(text)
       // $.POST("/message",{usuario:"en",message:text,file:""});
        _box_message.append('<div class="message out no-avatar media">' +
            '<div class="body media-body text-left p-l-50"><div class="content msg-reply f-12 bg-primary d-inline-block">'+ text +'</div><div class="seen"><i class="icon-clock f-12 m-r-5 txt-muted d-inline-block"></i><span><p class="d-inline-block">a few seconds ago </p></span><div class="clear"></div> </div></div>' +
